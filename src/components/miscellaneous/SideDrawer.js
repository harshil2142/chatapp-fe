...
const handleSearch = async()=>{
  if(!search){
    ...
    try {
      ...
      const {data} = await axios.get(`${baseApiUrl}/api/user?search=${search}`,config)
      ...
      toast({
        title: "Error Occured",
        description: "failed to load a search results",
        status: "error",
        duration: 5000,
        isClosable: true,
...
try {
  ...
  const {data} = await axios.post(`${baseApiUrl}/api/chat`,{userId},config)
        ...
        <div>
          <Menu>
            ...
            <MenuList pl={2}>
              ...
              {notification.length > 0 && notification?.map((notif,idx)=>(
